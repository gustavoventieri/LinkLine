...
public class ChatServiceImpl implements ChatService {
    ...
    public void createPrivateChat(final UUID currentUserId, final String targetUsername) {
        ...
        final UserDomain targetUser = userRepository.findByUsername(targetUsername)
                .orElseThrow(() -> new NotFound("Usuário alvo '" + targetUsername + "' não encontrado."));
        ...
        friendshipRepository.findExisting(currentUserId, targetUser.id(), List.of(RequestStatus.ACCEPTED))
                .orElseThrow(() -> new NotFound("Não existe uma amizade aceita com este usuário."));

        chatRepository.findExistingChat(currentUser.id(), targetUser.id())
                .ifPresent(existingChat -> {
                    log.warn("Tentativa de criar um chat duplicado entre {} e {}", currentUser.id(), targetUser.id());
        ...
        log.info("Chat privado criado com sucesso entre {} e {}", currentUser.username(), targetUser.username());
    }
        ...
        final List<PrivateChatResponseDTO> formattedChats = chats.stream()
                .map(chat -> {
                    ...
                    final UserDomain otherParticipant = chat.participants().stream()
                            ...
                            .findFirst()
                            // Lança uma exceção se um chat privado não tiver outro participante,
                            // o que indica um estado de dados inconsistente.
                            .orElseThrow(() -> new IllegalStateException(
                                    "Chat privado " + chat.id() + " não tem um segundo participante."));
