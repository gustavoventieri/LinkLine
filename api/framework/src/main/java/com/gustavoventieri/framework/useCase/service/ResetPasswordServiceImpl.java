...
public class ResetPasswordServiceImpl implements ResetPasswordService {
    ...
    public void initiateResetPassword(final String email) {
        ...
        persistResetPasswordRecord(email, record.code(), false, record.expiresAt());
    ...
    public void validateResetPasswordCode(final String email, final String code) {
        ...
        persistResetPasswordRecord(email, record.code(), true, record.expiresAt());
     ...
     * This method is only allowed if there is a valid and non-expired reset
     * password request.
     * After the update, the reset request is removed and the user is notified by
     * email.
     *
    ...
    public void updateUserPasswordByEmail(final String email, final String newPassword) {
        final ResetPasswordDomain record = resetPasswordRepository.findByEmail(email)
                .filter(ResetPasswordDomain::verified)
        ...
        sendPasswordUpdatedEmail(email, user.username());

        log.info("Password successfully updated for email: {}. Reset password record removed.", email);
    }

    /**
     * Sends a confirmation email to notify the user that the password has been
     * updated.
     *
     ...
     */
    private void sendPasswordUpdatedEmail(final String email, final String username) {
        try {
     ...
     */
    private void persistResetPasswordRecord(final String email, final String code, final boolean verified,
            final Instant expiresAt) {
        resetPasswordRepository.saveOrUpdate(email, code, verified, expiresAt);
    }
